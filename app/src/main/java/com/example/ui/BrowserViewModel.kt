...
class BrowserViewModel(
    ...
    fun addAIChatMessage(role: String, content: String) {
        try {
            ...
            val historyArray = try {
                org.json.JSONArray(historyString)
            } catch (e: Exception) {
                org.json.JSONArray()
            }
    ...
    fun performSearchFromAI(query: String) {
        val searchEngine = settings.value.searchEngine
        val searchUrl = when (searchEngine) {
            "Google" -> "https://www.google.com/search?q="
            "DuckDuckGo" -> "https://duckduckgo.com/?q="
            "Bing" -> "https://www.bing.com/search?q="
            "Yahoo" -> "https://search.yahoo.com/search?p="
            else -> "https://search.stormx.ninja/search?q="
        }
        ...
        navigateActiveTabFromAI("$searchUrl$encodedQuery")
    }

    fun navigateActiveTabFromAI(url: String) {
        val safeUrl = if (url.startsWith("http://")) {
            url.replaceFirst("http://", "https://")
        } else if (!url.startsWith("https://") && !url.startsWith("file://") && !url.startsWith("chrome://")) {
            "https://$url"
        } else {
            url
        }
        webViewMap[_activeTabId.value]?.loadUrl(safeUrl)
    }

    suspend fun getActiveTabContent(): String = kotlinx.coroutines.withTimeoutOrNull(3000L) {
        kotlinx.coroutines.suspendCancellableCoroutine { cont ->
            ...
            mainHandler.post {
                wv.evaluateJavascript("(function() { var text = ''; if (document.body && document.body.innerText) text = document.body.innerText; else if (document.documentElement && document.documentElement.innerText) text = document.documentElement.innerText; return text.substring(0, 10000); })();") { result ->
                    try {
                        var text = result ?: ""
    ...
    fun addNewTab(url: String = "https://search.stormx.ninja/") {
        val safeUrl = if (url.startsWith("http://")) {
            url.replaceFirst("http://", "https://")
        } else if (url.isNotBlank() && !url.startsWith("https://") && !url.startsWith("file://") && !url.startsWith("chrome://")) {
            "https://$url"
        } else {
            url
        }
        viewModelScope.launch {
            val title = "New Tab"
            val id = repository.addTab(safeUrl, title)
            val newTab = TabState(id = id, url = safeUrl, title = title)
            _tabs.value = _tabs.value + newTab
            _activeTabId.value = id
            _currentUrlInput.value = safeUrl
